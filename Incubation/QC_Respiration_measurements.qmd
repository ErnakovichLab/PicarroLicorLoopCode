...
fp_metadata <- "/examples/incubation/26SEI_JarLabelingandLoadingSpreadsheet.xlsx"

# location of sample respiration spreadsheets
fp_licor <- "~/Downloads/licor_data"

# location of sample respiration spreadsheets
fp_picarro <- "~/Downloads/OneDrive_1_1-20-2026/" # temporary; eventually move to examples
```
...
#' Takes a folder containng either Picarro or Licor-style respiration templates
#' and reads them in, does baic cleaning functions on them. 
#' @param resp_file_path_licor a directory path where identically formated respiration spreadsheets for the licor are housed. If you do not have data from the instrument, leave argument blank.
...
#' @param incubation_start A character that can be coerced into a datetime. If not specified, defaults to earliest datetime in respiration measurements
#' @return a data frame.
#' @examples
read_respiration_files <- function(resp_file_path_licor = NULL,
                                   resp_file_path_picarro = NULL, 
                                   sample_id_column = "Jar Number",
                                   incubation_start=NULL,
                                   file_pattern = ".xlsx") {
  ...
  # Initial read-in
  if(!is.null(resp_file_path_licor)) {
  ...
  if(!is.null(resp_file_path_picarro)) {
  ...
  # Calculate durations and jar pressures
    resp_raw <- resp_raw %>%  
      mutate(Date = as_date(Timestamp),
             IncubationDay = Date - as_date(incubation_start),
             IncubationTime = Timestamp - as_datetime(incubation_start, tz = "America/New_York"),
...
jar_info_raw <- readxl::read_excel(here(fp_metadata), sheet = 8, na = c("nan", "#VALUE!", "#DIV/0!")) %>%
  select(3:15)
...
-- Hannah notes to change: 1) turn into function 2) remove dosing 3) add readings to filter option to function
