...
fp_licor <- "examples/incubation/licor_data"
```
...
gsub("(.{1,})(-Respiration.xlsx)","\\1", basename(Resp_files))
...
read_respiration_files <- function(resp_file_path_licor = NULL,
  ...
  if(!is.null(resp_file_path_licor)) {
    Resp_files <- list.files(here(resp_file_path_licor),
                             pattern = "-Respiration.xlsx", full.names = T)
    ...
    col_types_licor <- c(rep("text", 2), #SampleType:JarNumber
                       ...
                       "guess","guess", "guess") # standard curve calcs
    
    resp_raw_licor <- map2_dfr(Resp_files, 
    ...
    # HANNAH start here - change names so that licor and picarro respiration sheets have same names.
  ...
  if(!is.null(resp_file_path_picarro)) {
    ...
    # Read in data
    resp_raw_picarro <- map2_dfr(Resp_files, 
                               ~readxl::read_excel(.x,sheet = 1, skip = 1,
                                                   col_types = col_types_picarro,
                                                   na = c("nan", "#VALUE!", "#DIV/0!"))) %>% 
      filter(!is.na(SampleType)) %>%
      select(SampleType:`True delta (permil)`) 
        # HANNAH start here - change names so that licor and picarro respiration sheets have same names.
  }
