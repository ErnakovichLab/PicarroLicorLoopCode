...
#### Reading in Data and initial QC ##############################################
...
std_curve_estimation <- std_curve_raw %>%
  # Clean and filter out columns and rows that are unnecessary for calibration calculations
  slice(start_row:end_row) %>%
  filter(!(SampleType %in% exclude_sampletypes)) %>% # remove rows where mistakes were made and noted in the sheet
  select(`Jar Number`,
         ...
         contains("True")) %>%
  filter(!is.na(`Time of start of measurement (write at least every 5 samples)`)) %>% # remove rows where time of sample measurement was not written in the sheet
  # Create derived columns that calculate what readings should be showing after sample injection under perfectly calibrated conditions
  ...
  group_by(`Jar Number`) %>%
  # Calculate outlier status based on normal distribution assumptions
...
Use the plots in this section to perform quality control on your data.
 ...
 - 
```{r}
samples_to_filter <-  c(32,11,6, 19)
samples_to_filter <- c()
std_curve_outlier_removal <- std_curve_estimation %>%
  filter(!(SampleNumber %in% samples_to_filter)) #%>%
